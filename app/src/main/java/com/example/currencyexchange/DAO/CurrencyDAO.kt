...
interface CurrencyDAO {
    ...
    suspend fun insertDefaultCurrency(databaseModel: CurrencyNamesModel)
    ...
    @Query("SELECT currency_name, is_base FROM currency_names")
    fun getCurrencies(): Flow<List<CurrencyNamesModel>>

    //  Get base currency from the database
    @Query("SELECT currency_name FROM currency_names WHERE is_base = 1")
